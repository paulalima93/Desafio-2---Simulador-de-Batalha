...
function actions.build()
    ...
    local specialAttack = {
        ...
        requirement = nil,
        execute = function(playerData, creatureData)
            local attackName = (creatureData.name == "Rattata") and "Mordida"
                or (creatureData.name == "Pidgey") and "Rajada de Vento"
                or (creatureData.name == "Meowth") and "Garras de Fúria"
            ...
            print(string.format("%s usou %s e deu %d pontos de dano", creatureData.name, attackName, damage))
            local healthRate = math.floor((playerData.health / playerData.maxHealth) * 10)
            print(string.format("%s: %s", playerData.name, utils.getProgressBar(healthRate)))
        end
    }
    ...
    -- Populate list
    actions.list[#actions.list + 1] = tackle
    actions.list[#actions.list + 1] = specialAttack
